-- Database Cleanup and Setup Script for Giza (SAFE VERSION)
-- This script safely removes unnecessary tables/functions and sets up only what's needed

-- =====================================================
-- STEP 1: DROP UNNECESSARY TABLES AND FUNCTIONS (SAFE)
-- =====================================================

-- Drop old/unused tables only if they exist (using safe checks)
DO $$ 
BEGIN
    -- Check and drop each table individually
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'participants') THEN
        DROP TABLE public.participants CASCADE;
    END IF;
    
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'shared_contracts') THEN
        DROP TABLE public.shared_contracts CASCADE;
    END IF;
    
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'contract_templates') THEN
        DROP TABLE public.contract_templates CASCADE;
    END IF;
...
-- Drop unnecessary RPC functions (safe drop with CASCADE)
...
-- Drop old triggers if they exist (safe drop)
...
-- =====================================================
-- STEP 2: DROP EXISTING CORE TABLES TO RECREATE CLEAN
-- =====================================================

-- Drop existing core tables to ensure clean structure
...
BEGIN
    ...
    -- Invoices policies
    DROP POLICY IF EXISTS "Users can view invoices in their rooms" ON public.invoices;
    DROP POLICY IF EXISTS "Users can create invoices in their rooms" ON public.invoices;
    DROP POLICY IF EXISTS "Users can update invoices in their rooms" ON public.invoices;
END $$;
...
-- =====================================================
-- STEP 8: POPULATE PROFILES FOR EXISTING USERS
-- =====================================================
...
BEGIN 
    ...
    RAISE NOTICE 'Tables created: profiles, rooms, documents, signatures, invoices';
    RAISE NOTICE 'All unnecessary tables and functions have been removed';
END $$;